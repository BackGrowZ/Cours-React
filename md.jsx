...
const FormatageCours = ({ cours }) => {
  const [coursSplit, setCoursSplit] = useState([]);

  useEffect(() => {
    const test = cours.split("[jsx]");
    setCoursSplit(test);
  }, [cours]);
  ...
  return (
    <>
      {coursSplit.map((e, i) => {
        if (i % 2 === 0) {
          return <MarkdownComponent markdownText={e} />;
        } else {
          return <FormatCode code={e} />;
        }
      })}
    </>
  );
